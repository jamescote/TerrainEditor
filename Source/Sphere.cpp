	...
	: Object3D( pPos, lID, sTexName, pAnimTrack )
{
	...
	m_iVertexBuffer = pShdrMngr->genVertexBuffer( m_iVertexArray, 0, 3, m_pVertices.data(), m_pVertices.size() * sizeof( vec3 ), GL_STATIC_DRAW );
	m_iNormalBuffer = pShdrMngr->genVertexBuffer( m_iVertexArray, 1, 3, m_pNormals.data(), m_pNormals.size() * sizeof( vec3 ), GL_STATIC_DRAW );
	m_iTextureBuffer = pShdrMngr->genVertexBuffer( m_iVertexArray, 2, 2, m_pUVs.data(), m_pUVs.size() * sizeof( vec2 ), GL_STATIC_DRAW );
